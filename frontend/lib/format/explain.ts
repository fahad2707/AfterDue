...
export function explainCase(input: {
  ...
  const opening = `RECLAIM opened this recovery case because the subscription returned to ACTIVE after a halted period containing ${input.invoiceCount} unpaid invoice${input.invoiceCount === 1 ? "" : "s"} totaling ${formatPaiseINR(input.backlogPaise)}.`;
